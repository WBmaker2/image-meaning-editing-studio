    ...
    <div className="app-shell">
        ...
        <main className="tutorial-screen">
          ...
          <section className="tutorial-card">
            ...
            <div className="tutorial-actions">
              ...
              <button className="primary-button" onClick={() => tutorialStep === 2 ? setScreen("cases") : setTutorialStep((step) => step + 1)}>{tutorialStep === 2 ? "사건 선택하기" : "다음 약속"} →</button>
            </div>
          </section>
          <button className="skip-button" onClick={() => setScreen("cases")}>안내를 건너뛰고 사건 보기</button>
        </main>
      )}
      ...
      {screen === "cases" && (
        <main className="case-screen">
          ...
          {reports.length > 0 && <button className="report-button" onClick={() => setScreen("report")}>나의 이미지 의미 비교 기록 보기 <span>{reports.length}</span></button>}
        </main>
      )}

      {screen === "workspace" && <Workspace learningCase={activeCase} onExit={() => setScreen("cases")} onSave={saveReport} onReadingHelp={() => setDialog("reading")} />}
