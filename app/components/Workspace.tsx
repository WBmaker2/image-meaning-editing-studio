...
export function Workspace({ learningCase, onExit, onSave, onReadingHelp }: WorkspaceProps) {
  ...
  const phaseSectionRef = useRef<HTMLElement | null>(null);
  const phaseHasMounted = useRef(false);
  useEffect(() => {
    if (!phaseHasMounted.current) {
      phaseHasMounted.current = true;
      return;
    }
  ...
  const checkAnswer = () => {
    ...
    if (!changes.length || !effect || !evidence || !confidence) {
      setFeedback(`아직 고르지 않은 곳: ${missingAnswers.join(" · ")}. 위에서부터 하나씩 골라요.`);
      return;
    }
    if (preset.blocksCompletion) {
      setFeedback("읽기 어려운 점은 잘 찾았어요. 다만 지금 보기는 글자가 잘 안 보여서 기록할 수 없어요. ‘편집으로 돌아가기’를 눌러 글자가 잘 보이게 바꿔 보세요.");
      setSuccess(false);
      return;
    }
    ...
    if (!changeOkay) setFeedback("처음과 바꾼 그림에서 크기·자리·글·색 중 무엇이 달라졌는지 다시 찾아봐요.");
    else if (!effectOkay) setFeedback("찾은 변화와 이어지는 느낌이나 뜻을 다시 골라요. 같은 편집에도 맞는 답이 여러 개일 수 있어요.");
    else if (!evidenceOkay) setFeedback("느낌과 뜻은 잘 골랐어요. 이제 그 생각을 보여 주는 화면 속 크기·자리·글·색의 차이를 찾아요.");
    else if (!confidenceOkay) setFeedback(confidence === "overclaim" ? "그 생각의 까닭이 화면에 있어요. 사진 밖의 일을 사실처럼 말한 것은 아닌지, 가능한 느낌인지 구분해요." : "사람마다 다르게 느낄 수 있는 말인지, 화면에서 바로 볼 수 있는 말인지 다시 생각해요.");
    else {
      setFeedback("좋아요! 바꾼 점과 그렇게 느낀 까닭을 잘 연결했어요.");
      setSuccess(true);
      moveToPhase("result");
    }
  };
  ...
  return (
    <main className="workspace-screen">
      <div className="workspace-topline">
        <button className="back-link" onClick={onExit}>← 활동 모음</button>
        <div className="case-progress"><span>활동 0{learningCase.number}</span><strong>{learningCase.focus}</strong></div>
        <button className="help-link" onClick={onReadingHelp}>이미지 읽기 방법</button>
      </div>
      <section className="workspace-heading">
        <div><p className="eyebrow">오늘의 편집 활동</p><h1>{learningCase.title}</h1><p>{learningCase.mission}</p></div>
        <div className="purpose-note"><span>이 편집의 목표</span><strong>{learningCase.purpose}</strong></div>
      </section>
      ...
      {phase === "observe" && (
        <section className="observe-layout" ref={phaseSectionRef}>
          ...
          <aside className="panel observation-panel">
            ...
            <button className={`primary-button full-button ${observationComplete ? "gi-pulse" : ""}`} disabled={!observationComplete} onClick={() => moveToPhase("edit")}>{observationComplete ? "처음 관찰을 마쳤어요 →" : `화면 단서 ${learningCase.objects.length - observedObjects.length}개를 더 찾아요`}</button>
          </aside>
        </section>
      )}
      ...
      {phase !== "observe" && (
        <>
          <div className="comparison-toolbar">
            <div className="segmented-control" role="group" aria-label="이미지 비교 방법"><button className={viewMode === "side" ? "active" : ""} aria-pressed={viewMode === "side"} onClick={() => setChosenViewMode("side")}>▥ 나란히 보기</button><button className={viewMode === "alternate" ? "active" : ""} aria-pressed={viewMode === "alternate"} onClick={() => setChosenViewMode("alternate")}>◫ 한 장씩 보기</button></div>
      ...
      {phase === "edit" && (
        <section className="tool-panel panel" ref={phaseSectionRef}>
          ...
          <div className="tool-actions"><button className="secondary-button" onClick={() => moveToPhase("observe")}>처음 이미지 다시 보기</button><button className="primary-button gi-pulse" onClick={() => moveToPhase("analyze")}>느낌과 까닭 살펴보기 →</button></div>
        </section>
      )}

      {phase === "analyze" && (
        <section className="analysis-grid" ref={phaseSectionRef}>
          ...
          <div className="analysis-actions"><button className="secondary-button" onClick={() => moveToPhase("edit")}>편집으로 돌아가기</button><button className="primary-button gi-pulse" onClick={checkAnswer}>연결한 생각 확인하기 →</button></div>
          {feedback && <div className={`feedback ${success ? "success" : ""}`} role="status"><strong>{success ? "근거 연결 완료" : "한 번 더 살펴봐요"}</strong><p>{feedback}</p></div>}
        </section>
      )}

      {phase === "result" && (
        <section className="result-layout" ref={phaseSectionRef}>
          <div className="result-main panel">
            <p className="eyebrow">이미지 비교 기록</p><h2>바꾼 점과 그렇게 느낀 까닭을 연결했어요</h2>
          ...
          <div className="result-actions"><button className="secondary-button" onClick={() => { moveToPhase("edit"); setSuccess(false); }}>한 요소 다시 편집하기</button><button className="primary-button gi-pulse" onClick={save}>이 활동을 기록에 담기 →</button></div>
        </section>
      )}
    </main>
  );
